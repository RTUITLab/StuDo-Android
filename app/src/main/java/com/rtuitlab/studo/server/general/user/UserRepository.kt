...
class UserRepository (
    ...
    suspend fun changeUserInfo(changeUserInfoRequest: ChangeUserInfoRequest): Resource<User> {
        return try {
            responseHandler.handleSuccess(userApi.changeUserInfo(changeUserInfoRequest))
        } catch (e: Exception) {
            responseHandler.handleException(e)
        }
    }
